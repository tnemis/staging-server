...
class Student_schemesView(View): 
    def get(self,request,**kwargs):
        ...
        classwise_detail = Child_detail.objects.filter(school=schl_id,class_studying_id=class_id).exclude(transfer_flag = 1).order_by('id')
        student_schemes_list=Student_schemes.objects.filter(school=schl_id,cls=class_id).order_by('student')
        if class_id == '1':
    ...
    def post(self,request,**kwargs):
        ...
        if student_schemes_list:
            # for s in student_schemes_list:
            #     new_id=s.id
            #     clss=s.cls
                                

            for u in range(len(student_schemes_list)):
                school_key=request.POST.get('school_'+str(u+1))
                child_key=request.POST.get('child_'+str(u+1))
                scheme_detail = Student_schemes(
                    id=student_schemes_list[u].id,
                    school=School.objects.get(id=school_key),
                    academic_year=request.POST.get('academic_year_'+str(u+1)),
                    student=Child_detail.objects.get(id=child_key),
                    cls=student_schemes_list[u].cls,
                    uniform_1=request.POST.get('uniform_1_'+str(u+1)),
                    uniform_2=request.POST.get('uniform_2_'+str(u+1)),
                    uniform_3=request.POST.get('uniform_3_'+str(u+1)),
                    uniform_4=request.POST.get('uniform_4_'+str(u+1)),
                    textbook=request.POST.get('textbook_'+str(u+1)),
                    textbook_1=request.POST.get('textbook_1_'+str(u+1)),
                    textbook_2=request.POST.get('textbook_2_'+str(u+1)),
                    textbook_3=request.POST.get('textbook_3_'+str(u+1)),
                    notebook=request.POST.get('notebook_'+str(u+1)),
                    notebook_1=request.POST.get('notebook_1_'+str(u+1)),
                    notebook_2=request.POST.get('notebook_2_'+str(u+1)),
                    notebook_3=request.POST.get('notebook_3_'+str(u+1)),
                    bag=request.POST.get('bag_'+str(u+1)),
                    footware=request.POST.get('footware_'+str(u+1)),
                    sweater=request.POST.get('sweater_'+str(u+1)),
                    crayon=request.POST.get('crayon_'+str(u+1)),
                    colorpencil=request.POST.get('colorpencil_'+str(u+1)),
                    geometrybox=request.POST.get('geometrybox_'+str(u+1)),
                    atlas=request.POST.get('atlas_'+str(u+1)),
                    cycle=request.POST.get('cycle_'+str(u+1)),
                    laptop=request.POST.get('laptop_'+str(u+1)),
                    bw=request.POST.get('bw_'+str(u+1)),
                    sci=request.POST.get('sci_'+str(u+1)),
                    laptop_no=request.POST.get('laptop_no_'+str(u+1)),
                    laptop_date=request.POST.get('laptop_date_'+str(u+1)),
                    created=student_schemes_list[u].created,)
                scheme_detail.save()
            msg = "Class- " + str(class_id) + "  Schemes detail updated successfully"
...
class Schemes_class_wise_count(View):
    ...
    def get(self,request,**kwargs):
        try:
            ...
            return render(request,'schemes/count_table.html')
